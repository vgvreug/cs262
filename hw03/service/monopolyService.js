...
router.get("/DogScores", readDogScores);
...
function readDogScores(req, res, next){
    db.many("SELECT score FROM Player, PlayerGame, Game WHERE Player.ID = PlayerGame.playerID AND PlayerGame.gameID = Game.ID AND name LIKE '%Dogbreath%' ORDER BY time Desc")
        .then(data => {
            res.send(data);
        })
        .catch(err => {
